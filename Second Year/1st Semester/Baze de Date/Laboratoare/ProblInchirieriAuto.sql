...
insert into Marci(denumire)values('Dacia'),('Citroen'),('VW')
...
insert into Clienti(nume, prenume)values ('Popa','Ionut'),('Rusu','Maria'),('Cosma','Alexandru')
...
insert into Angajati(nume, prenume)values ('Nistor','Vasile'),('Simion','Andrei'),('Moldovan','Claudiu')
...
insert into Autovehicule(numar,motorizare,mid)values('SV10ABC','disel',1),('SV12FHL','benzina',2),('CJ10IOI','motorina',3),('B100JOY','gaz',1)
...
insert into Inchirieri(aid,numar,cid,d_inchiriere,d_returnare)values(1,'SV10ABC',1,'2019-01-03 10:00:00','2019-01-03 20:00:00'),(2,'B100JOY',2,'2019-01-03 10:00:00','2019-01-05 20:00:00'),(3,'CJ10IOI',3,'2019-01-06 10:00:00','2019-01-07 10:00:00')
...
-- 2) Creati o procedura stocata care primeste un angajat, un autovehicul, un client, data si ora inchirierii si data si ora returnarii si 
-- un parametru tip operatie de tip bit. Daca tip operatie este true atunci se insereaza un rand in registrul de inchirieri, altfel se actualizeaza 
-- data si ora inchirierii, respectiv data si ora returnarii.
...
select A.nume +' '+A.prenume as 'Nume', count(I.id) as 'Nr inchirieri', M.denumire as 'Marca'
	...
	having count(I.id)>=1
select Nume, [Nr inchirieri] from vw_Angajati where Marca='Dacia'
go
...
create or alter function uf_autovehicule(@moment datetime)
returns table
as
return 

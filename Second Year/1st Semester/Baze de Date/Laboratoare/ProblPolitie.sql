...
AS
	SELECT P.nume AS numeP, S.denumire AS numeS, COUNT(*) AS nrOre FROM Politisti P INNER JOIN PolitistiSectie PS ON PS.politist=P.id
...
AS
	SELECT P.nume AS numeP,  COUNT(*) AS nrOre FROM Sectii S INNER JOIN PolitistiSectie PS ON PS.sectie=S.id
																				 INNER JOIN Politisti P ON PS.politist=P.ID
																				 GROUP BY P.nume
GO

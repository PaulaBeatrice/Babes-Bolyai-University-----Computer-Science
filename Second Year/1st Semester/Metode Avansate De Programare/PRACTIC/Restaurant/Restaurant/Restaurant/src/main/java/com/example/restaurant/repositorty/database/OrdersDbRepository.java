...
import com.example.restaurant.utils.observer.ConcreteObservable;
...
import static com.example.restaurant.utils.utils.ChangeEventType.ADD;
import static com.example.restaurant.utils.utils.ChangeEventType.UPDATE;

public class OrdersDbRepository extends ConcreteObservable<OrderEvent> implements Repository<Integer, Orders> {
    ...
    private Orders getOrderFromResultSet(ResultSet resultSet) throws SQLException {
        ...
        do {
            ...
            orderItems.add(new OrderItems(new MenuItem(item_id, item_category, item_item, item_price, item_currency), item_quantity));
        } while(resultSet.next());

        return new Orders(order_id, new Table(table_id), order_date, orderItems, orderStatus);
    }

...
namespace Laborator_1.repository
{
    public class ResultDBRepository:ResultRepository
    {
        ...
        public int isEvaluated(Participant participant, Referee r)
        {
            log.InfoFormat("Entering isEvaluated with value {0}", participant);
            IDbConnection conn = DBUtils.getConnection(props);
            List<Participant> participants = new List<Participant>();
            using (var command = conn.CreateCommand())
            {
                command.CommandText = "select id_participant from Results where id_referee=@ref and id_participant=@part";
                IDbDataParameter paramRef = command.CreateParameter();
                paramRef.ParameterName = "@ref";
                paramRef.Value = r.Id;

                IDbDataParameter paramPart = command.CreateParameter();
                paramPart.ParameterName = "@part";
                paramPart.Value = participant.Id;

                command.Parameters.Add(paramRef);
                command.Parameters.Add(paramPart);
                ...
                using (var dataR = command.ExecuteReader())
                {
                    ...
                    {
                        int idP = dataR.GetInt32(0);
                        Participant p = participantRepository.FindOne(idP);
                        log.InfoFormat("Existing isEvaluated with value {0}", participant);
                        participants.Add(participant);
                    }
                }
            }
        ...
        public IEnumerable<Participant> GetPartsByActivity(string activity)
        {
            log.InfoFormat("Entering GetPartsByActivity with value {0}", activity);
            IDbConnection conn = DBUtils.getConnection(props);
            List<Participant> participants = new List<Participant>();
            using (var command = conn.CreateCommand())
            {
                command.CommandText = "select id_participant from Results where activity=@act";
                IDbDataParameter paramAct = command.CreateParameter();
                paramAct.ParameterName = "@act";
                paramAct.Value = activity;
                command.Parameters.Add(paramAct);
                ...
                using (var dataR = command.ExecuteReader())
                {
                    while (dataR.Read())
                    {
                        int idP = dataR.GetInt32(0);
                        Participant participant = participantRepository.FindOne(idP);
                        log.InfoFormat("Existing GetPartsByActivity with value {0}", activity);
                        participants.Add(participant);
                    }
                }
            }

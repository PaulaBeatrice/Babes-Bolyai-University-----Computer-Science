...
void StoreGUIModele::connectSignalsSlots()
{
	...
	QObject::connect(lstV->selectionModel(), &QItemSelectionModel::selectionChanged, [this]() {
		...
		int selRow = lstV->selectionModel()->selectedIndexes().at(0).row();
		auto cel0Index = lstV->model()->index(selRow, 0);
		auto cel0Value = lstV->model()->data(cel0Index, Qt::DisplayRole).toString();
		editDenumire->setText(cel0Value);
		auto cel1Index = lstV->model()->index(selRow, 1);
		auto cel1Value = lstV->model()->data(cel1Index, Qt::DisplayRole).toString();
		editDestinatie->setText(cel1Value);
		auto cel2Index = lstV->model()->index(selRow, 2);
		auto cel2Value = lstV->model()->data(cel2Index, Qt::DisplayRole).toString();
		editTip->setText(cel2Value);
		auto cel3Index = lstV->model()->index(selRow, 3);
		auto cel3Value = lstV->model()->data(cel3Index, Qt::DisplayRole).toString();
		editPret->setText(cel3Value);
		});
	...
	QObject::connect(btnCosCRUGUI, &QPushButton::clicked, [&]() {
		auto fereastraCos = new CosGUICuLista{ ctr.getCos() };
		fereastraCos->show();
		});

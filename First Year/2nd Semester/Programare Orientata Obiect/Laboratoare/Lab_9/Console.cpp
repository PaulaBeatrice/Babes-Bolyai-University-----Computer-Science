...
void UI::UI_printOferte( vector<Oferta>& allOferte) {
	try {
		if (allOferte.size() == 0)
...
void UI::UI_printOferteCos(const vector<Oferta>& allOferte)
{
	try {
		if (allOferte.size() == 0)
			cout << "Nu exista oferte." << endl;
...
void UI::UI_Cos() {
	...
	while (runProgram) {
		...
		case 3:
			...
			break;
		case 4:
			UI_printOferteCos(srv.getAllOferteCos());
			break;
		case 5:
			UI_export();
			break;

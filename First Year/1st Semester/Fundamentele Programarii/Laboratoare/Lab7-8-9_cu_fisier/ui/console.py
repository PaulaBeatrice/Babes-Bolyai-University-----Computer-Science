...
from exceptions.exceptions import ValidationException, DuplicateIDException, BookNotFoundException, \
    ClientNotFoundException, RentAlreadyAssignedException, RentItemNotFoundException, RentItemAlreadyReturned
...
class Console:
    ...
    def __show_all_books(self):
        ...
        if len(books) == 0:
            ...
            for book in books:
                print('Id:', colored(book.getId(), 'cyan'), '-Titlu:', colored(book.getTitlu(), 'cyan'), '-Descriere:',
                       colored(book.getDescriere(), 'cyan'), '-Autor:', colored(book.getAutor(), 'cyan'),
                      '-Inchiriata:', colored(book.getInchiriata(), 'cyan'))
    ...
    def __show_all_clients(self):
        ...
        if len(clients) == 0:
            ...
            for client in clients:
                print('Id:', colored(client.getId(), 'cyan'), '-Nume:', colored(client.getNume(), 'cyan'), '-Cnp:',
                      colored(client.getCnp(), 'cyan'))
    ...
    def __show_list_of_books(self, books):
        ...
        if len(books) == 0:
            ...
            for book in books:
                print('Id:', colored(book.getId(), 'cyan'), '-Titlu:', colored(book.getTitlu(), 'cyan'), '-Descriere:',
                      colored(book.getDescriere(), 'cyan'), '-Autor:', colored(book.getAutor(), 'cyan'), 'Inchiriata:',
                      colored(book.getInchiriata(), 'cyan'))
    ...
    def __show_list_of_clients(self, clients):
        ...
        if len(clients) == 0:
            ...
            for client in clients:
                print('Id:', colored(client.getId(), 'cyan'), '-Nume:', colored(client.getNume(), 'cyan'), '-Cnp:',
                      colored(client.getCnp(), 'cyan'),  '-NrCartiInchiriate: ', colored(client.getNrCarti(), 'cyan'))
    ...
    def __update_book(self):
        ...
        descriere = input('Descrierea cartii: ')
        autor = input('Autorul cartii: ')
        inchiriata = input('Inchiriata(True - DA, False - NU): ')
        try:
            updated_book = self.__book_srv.update_book(id, titlu, descriere, autor, inchiriata)
    ...
    def __update_client(self):
        ...
        cnp = input('Cnp-ul clientului: ')
        try:
            updated_client = self.__client_srv.update_client(id, nume, cnp)
    ...
    def __add_rent_item(self):
        ...
        try:
            ...
            print('Rent item-ul', rent_item, 'a fost adaugat cu succes')
        except ValidationException as ve:
            print(colored(str(ve), 'red'))
        except BookNotFoundException as e:
            print(colored(str(e), 'red'))
    ...
    def __filter_by_name(self):
        ...
        filtered_list = []
        self.__client_srv.filter_by_name(substring, lista_clienti, 0, filtered_list)
    ...
    def __top_books(self):
        ...
        cnt = 1
        for ob in list_cl:
            print(str(cnt) + ') Carte: [Titlu]:', colored(ob.getTitluCarte(), 'cyan'), 'a fost inchiriata de ',
                  colored(ob.getNumarInchirieri(), 'cyan'), 'clienti')
            cnt += 1
    ...
    def show_ui(self):
        while True:
            ...
            print('INCHIRIERE CARTE COMENZI: rent_book, return_book, show_all_rent_items')
            print('RAPOARTE INCHIRIERI: top_books, rent_report_name, rent_report_books, top_clients, raport_lab')

...
def setup_test_repo():
    b1 = Book('1', 'A', 'fsdfnnb', 'Ana')
    b2 = Book('2', 'B', 'sgdfrdg', 'Barbu')
    b3 = Book('3', 'C', 'fsfsggg', 'Calin')
    b4 = Book('4', 'D', 'fetdbff', 'Denis')
    b5 = Book('5', 'E', 'fsargnm', 'Eric')
    b6 = Book('6', 'F', 'xcvbnnb', 'Florin')
    b7 = Book('7', 'G', 'fsnjinb', 'Georgiana')
    b8 = Book('8', 'H', 'mmmfnnb', 'Horia')
    b9 = Book('9', 'I', 'fsqwerb', 'Ioana')
    b10 = Book('10', 'L', 'ftgfnnb', 'Laura')
...
class BookRepoFile:
    ...
    def __load_from_file(self):
        ...
        lines = f.readlines()
        for line in lines:
            book_id, book_titlu, book_descriere, book_autor = [token.strip() for token in line.split(';')]
            book = Book(book_id, book_titlu, book_descriere, book_autor)
            books.append(book)
    ...
    def __save_to_file(self, books_list):
        with open(self.__filename, 'w') as f:
            for book in books_list:
                book_string = str(book.getId()) + ';' + str(book.getTitlu()) + ';' + str(book.getDescriere()) + ';' + str(book.getAutor()) + '\n'
                f.write(book_string)
...
def test_library_file():
    ...
    test_repo.librabry(Book('550', 'wve', 'fesd', 'efwrd'))

    assert (test_repo.size() == 1)

    try:
        test_repo.librabry(Book('550', 'asdv', 'vcx', ' vcd'))
        assert False
    except DuplicateIDException:
        assert True

    test_repo.librabry(Book('74', 'fh', 'dfgh', 'dff'))
    test_repo.librabry(Book('574150', 'wve', 'sd', 'sdfv'))
    test_repo.librabry(Book('662', 'wve', 'sd', 'sdv'))
    ...
    new_book = Book('574150', 'frgb', 'df', 'sdfv')
    new_book2 = Book('662', 'sdfv', 'sd', 'wdfv')
